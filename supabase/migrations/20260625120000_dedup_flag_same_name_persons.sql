-- Flag same-name people (different emails) for review.
--
-- People are keyed by email everywhere (Affinidad, Gmail, Notes), so one human
-- with two addresses is two `person` nodes with different canonical_keys. The
-- flag-writing loop's "declared-distinct" skip (migration 20260622150000) treats
-- *any* pair of differing non-null canonical_keys as intentionally distinct and
-- writes no duplicate_flag — which silently drops genuine same-person/two-email
-- candidates (e.g. two "Pablo Campos" nodes).
--
-- Change: in the flag loop, also read the candidate's label and EXEMPT persons
-- whose full name (first + surname) equals the incoming label from that skip, so
-- they DO get a `duplicate_flags` row (resolution='pending') for human review.
-- Review-only — no auto-merge (the auto-merge branch still refuses differing
-- canonical_keys). Everything else — thresholds, check_duplicates, the no-dupes
-- path, and the bare-email label upgrade — is unchanged from 20260624120000.
...
DECLARE
  ...
  v_cand_key TEXT; v_cand_label TEXT; v_flags_written INT := 0;
BEGIN
  ...
  LOOP
    ...
    -- Declared-distinct skip: differing non-null canonical_keys are intentionally
    -- distinct (e.g. per-occurrence event keys) — EXCEPT same-name persons, which
    -- we surface for review (one human, two emails).
