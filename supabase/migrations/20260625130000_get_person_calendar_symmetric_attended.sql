-- The calendar/notes connectors now model attendance symmetrically:
-- `person -attended-> event` for the owner AND every attendee (no `attended_by`).
-- Make get_person_calendar agnostic to edge direction/label so co-attendees still
-- resolve: a co-participant is the *other end* of any attendance/about edge that
-- touches the event, in either direction. related_events keeps the legacy
-- `attended_by` branch so pre-existing data still reads.
