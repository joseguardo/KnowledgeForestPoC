-- ============================================================================
-- Stage 2: insert_pointer_with_dedup stamps `acl` and UNIONS it on merge.
-- ----------------------------------------------------------------------------
-- Adds p_acl (the principal set for the new pointer; falls back to translating
-- p_access_class via principals_for_class). Every INSERT branch stamps
-- pointers.acl; the MERGED branch unions the incoming acl into the existing row
-- (this is what grows a shared person::{email} node's acl as a new firm meets
-- them). access_class_id is still stamped during the transition (unused by RLS).
-- The same-name-person duplicate-flag carve-out and the merge gating are
-- unchanged from 20260625120000.
-- ============================================================================
...
BEGIN
  ...
  IF v_dupes IS NULL OR jsonb_array_length(v_dupes) = 0 THEN
    INSERT INTO pointers (label, type, canonical_key, metadata, embedding, access_class_id, acl)
    VALUES (p_label, p_type, p_canonical_key, p_metadata, p_embedding, v_class_id, v_acl)
  ...
  THEN
    -- Grow the merged node's acl to include the incoming principals (e.g. a new
    -- firm meeting a shared person::{email}).
    UPDATE pointers
